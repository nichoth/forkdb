...
test('pull sync verify', async function (t) {
    t.plan(8);
    forkdb1.heads('blorp', function (_err, hs) {
        t.ifError(_err);
        t.deepEqual(hs, [{ hash: hashes[3] }]);
    });
    forkdb1.list(function (_err, hs) {
        t.ifError(_err);
        t.deepEqual(mhashes(hs).sort(), [hashes[0], hashes[1], hashes[2], hashes[3]].sort(), 'list a');
    });
    forkdb2.heads('blorp', function (_err, hs) {
        t.ifError(_err);
        t.deepEqual(hs, [{ hash: hashes[3] }]);
    });
    forkdb2.list(function (_err, hs) {
        t.ifError(_err);
        t.deepEqual(mhashes(hs).sort(), [hashes[1], hashes[2], hashes[3]].sort(), 'list b');
    });
});
function mhashes(xs) { return xs.map(function (x) { return x.hash; }); }
//# sourceMappingURL=pull_sync.js.map