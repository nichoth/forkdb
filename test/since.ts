...
const tmp = { a: path.join(tmpdir, 'a'), b: path.join(tmpdir, 'b') }
mkdirp.sync(tmp.a)
mkdirp.sync(tmp.b)
...
test('populate push sync', function (t) {
    ...
    (function next () {
        ...
        const w = fdb.a.createWriteStream(doc.meta, function (err, hash) {
            t.ifError(err)
    ...
    (function next () {
        ...
        const w = fdb.b.createWriteStream(doc.meta, function (err, hash) {
            t.ifError(err)
            t.equal(doc.hash, hash)
            next()
        })
...
test('since', function (t) {
    t.plan(2)
    const ra = fdb.a.replicate({ mode: 'sync' }, function (err, hs) {
        t.ifError(err)
    })
    const rb = fdb.b.replicate({ mode: 'sync' }, function (err, hs) {
        t.ifError(err)
    })
    ra.pipe(rb).pipe(ra)
})
...
test('since add another', function (t) {
    ...
    const w = fdb.a.createWriteStream(doc, function (err) {
        t.ifError(err)
    })
    w.end('woo')
})
