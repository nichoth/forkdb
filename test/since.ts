...
test('populate push sync', async function (t: any) {
    const docs = { a: [], b: [] }
    docs.a.push({
        hash: hashes[0]!,
        body: 'beep boop\n',
...
test('since', async function (t: any) {
    t.plan(2)
...
test('since verify', async function (t: any) {
    t.plan(4)
...
test('since add another', async function (t: any) {
    t.plan(1)
    const doc = {
        key: 'blorp',
        prev: [hashes[3]!],
...
test('since replicate sequence', async function (t: any) {
    t.plan(7)
    const ra = forkdb1.replicate({ mode: 'sync' }, function (_err: any, hs) {
        t.ifError(err)
    })
    ...
    rb.on('response', function (hash: any) {
        t.deepEqual(hash, hashes[4]!)
    })
    ra.pipe(rb).pipe(ra)
})

test('since verify after', async function (t: any) {
    t.plan(4)
