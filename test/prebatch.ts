...
test('prebatch', async function (t: any) {
    const opts = {
    ...
    (function next () {
        ...
        const w = forkdb.createWriteStream(doc.meta, opts, function (_err: any, hash: any) {
            t.ifError(err)
...
test('prebatch', async function (t: any) {
    ...
    check(t, forkdb, expected)
    forkdb.createReadStream(hashes[0]!).pipe(concat(function (body: any) {
        t.equal(body.toString('utf8'), 'beep boop\n')
    }))
    forkdb.createReadStream(hashes[1]!).pipe(concat(function (body: any) {
        t.equal(body.toString('utf8'), 'BEEP BOOP\n')
    }))
    forkdb.createReadStream(hashes[2]!).pipe(concat(function (body: any) {
        t.equal(body.toString('utf8'), 'BeEp BoOp\n')
    }))
    forkdb.createReadStream(hashes[3]!).pipe(concat(function (body: any) {
        t.equal(body.toString('utf8'), 'BEEPITY BOOPITY\n')
    }))

    const yo = forkdb.db.createReadStream({
        gt: ['yo', 'skrill', null],
        lt: ['yo', 'skrill', undefined]
    })
    yo.pipe(collect(function (rows: any[]) {
        t.deepEqual(rows, [
